...
@pytest.fixture
def mock_23andme_v5_is_generated_path() -> Path:
    """Path to the June 2026 v5 'is generated' banner-variant fixture.

    Pins the second canonical first-line banner variant 23andMe ships in
    newer v5 exports. Same body as ``mock_23andme.txt``; only the first
    line differs (``This data file is generated by 23andMe.``). Used to
    pin regression coverage on the banner-variant tolerance in the parser.
    """
    path = FIXTURES_DIR / "mock_23andme_v5_is_generated.txt"
    if not path.exists():
        pytest.fail(f"Mock fixture missing: {path}")
    return path
